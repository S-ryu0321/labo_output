...
def main():
    ...
    anom_df.to_csv("anom_1d_timeseries_MA.csv", header=None, index=False)
    print(anom_array)
    ...
    for i in range(len(anom_ls[0])):
        ...
        plt.colorbar(cs)
        plt.clim(0, 5)
        plt.title('anomaly_'+str(i+1)+'movin-average')
        plt.savefig(figdir+'anomaly'+str(i+1)+'_MA.png', format='png', dpi=200)
    '''
