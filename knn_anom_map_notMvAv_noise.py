...
def main():
    ...
    anom_df.to_csv("anom-noise_1d_timeseries_raw.csv", header=None, index=False)
    print(anom_array)
    ...
    for i in range(len(anom_ls[0])):
        ...
        plt.colorbar(cs)
        plt.clim(0, 5)
        plt.title('noise-anomaly_'+str(i+1)+'raw')
        plt.savefig(figdir+'anomaly'+str(i+1)+'_raw_noise.png', format='png', dpi=200)
